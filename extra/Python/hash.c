/* hash.c
   ...
   A hash table implementation derived from lp_hash.c from the lp_solve library
   to quickly translate a provided name to an index.
...
int hashval(const char *string, int size)
{
...
#if 0
int find_row(lprec *lp, char *name, MYBOOL Unconstrained_rows_found)
{
  hashelem *hp;
...
int find_var(lprec *lp, char *name, MYBOOL verbose)
{
  hashelem *hp;
